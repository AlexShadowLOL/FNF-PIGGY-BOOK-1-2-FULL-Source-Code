...
class DumbassFreeplay extends MusicBeatState
{
	...
	var freeplayComposers:Array<String> = [
		'alexshadow',         // no male bitches
		'alexshadow',         // friendzone
		'alexshadow'          // extermination
	];

	// funni bgs
	var bookBGS:Array<String> = [
		'nomalebitches',     // no male bitches
	...
	{
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('data/freeplaySonglist3'));

		for (i in 0...initSonglist.length)
		{
			var data:Array<String> = initSonglist[i].split(':');
			songs.push(new SongMetadata5(data[0], Std.parseInt(data[2]), data[1]));
		}

		#if windows
		// Updating Discord Rich Presence
		DiscordClient.changePresence("In the Freeplay Menu (Dumbass Page)", null);
		#end
	...
	{
	/*
		if (FlxG.save.data.unlockedRematch == false)
			curDifficulty = 0;
		else
	...
	*/
		...
		if (curDifficulty != 1)
			...
			FlxG.sound.playMusic(Paths.instRematch(songs[curSelected].songName), 1);
		#end
